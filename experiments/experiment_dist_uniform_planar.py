...
def main():
    ...
    dataset_name = "planar"
    experiment_name = "dist_uniform_planar_q20"

    if not os.path.isdir(os.path.join(parent_dir, "results_and_plots/KingsCollege", experiment_name)):
        os.makedirs(os.path.join(parent_dir, "results_and_plots/KingsCollege", experiment_name), exist_ok=True)
    
    datasets_folder = os.path.join(parent_dir, "models/KingsCollege/", dataset_name)
    results_folder = os.path.join(parent_dir, "results_and_plots/KingsCollege", experiment_name, "results")
    plots_folder = os.path.join(parent_dir, "results_and_plots/KingsCollege", experiment_name, "plots")    
    ...
    # mins = [0.3, 0.5]
    # maxs = [0.9, 0.7]
    inliers_min_max =  [[0.3, 0.8], [0.1, 0.9], [0.5, 0.7]]  #list(itertools.product(mins,maxs))
    ...
    experiment = "dist_uniform"
    experiment_param = ""   # experiment_param denotes whether the smallest inlier ratio should be assigned to closest or the farthest camera, both "0" and "1" will be assigned later
    ...
    title = "Planar scene, "
    plot_all([experiment], noises, results_folder, inliers_min_max, plots_folder, ["0", "1"], title)
