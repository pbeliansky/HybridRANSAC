...
#include <solvers/sh5f_3/cpp/sh5f_3.h>
#include <solvers/sh5f_3/cpp/sh5f_3_cf.h>
#include <solvers/sh5f_3/cpp/sh5f_3_cm.h>

#include "util.h"


namespace test_sh5f_3
{
	template <int config>
	...
	{
		if (config == 0)
			sh5f_3::solver_sturm(q, p, c, &fss, &Hs, &Nss);
		else if (config == 1)
			sh5f_3::solver_closed_form(q, p, c, &fss, &Hs, &Nss);
		else if (config == 2)
			sh5f_3::solver_companion_matrix(q, p, c, &fss, &Hs, &Nss);
	}
}
#pragma once