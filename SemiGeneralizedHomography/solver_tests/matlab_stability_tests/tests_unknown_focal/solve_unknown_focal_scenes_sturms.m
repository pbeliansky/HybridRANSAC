...
% % noise_levels =[10.0];
% % planar noise
% noise_levels = [0, 1e-3, 1e-2, 0.1, 0.5];
% 
...
% boxplot_noise_rot(errors(1:2), noise_levels, solver_names, 'random plane with noise');
% boxplot_noise_tn(errors(1:2), noise_levels, solver_names, '');
% boxplot_noise_f(errors(1:2), noise_levels, solver_names, '');
% 
% solver_names = { 'sH5f_3_c' 'sH5f_3_s' 'sH5f_3_{cf}'}
% boxplot_noise_rot(errors(3:5), noise_levels, solver_names, 'random plane with noise');
% boxplot_noise_tn(errors(3:5), noise_levels, solver_names, '');
% boxplot_noise_f(errors(3:5), noise_levels, solver_names, '');
% 
% solver_names = { 'Ef6+1' 'Ef6+1_d'  'Ef6+1_{mex}' 'Ef6+1_d_{mex}' }
% boxplot_noise_rot(errors(6:9), noise_levels, solver_names, 'random plane with noise');
% boxplot_noise_tn(errors(6:9), noise_levels, solver_names, '');
% boxplot_noise_f(errors(6:9), noise_levels, solver_names, '');
...
tols = size(tol,2);
for t=1:tols
    solver_names{t} = ['sH5f_2_{s', num2str(tol(t)), '}'];
end
histogram_rot(errors(1:tols), [0], solver_names, "Rotation error frequency in "+int2str(iterations)+" iterations", 'sturm_histograms\histogram_rot_sH5f_2_sturms.pdf');
histogram_tn(errors(1:tols), [0], solver_names, "Translation error frequency in "+int2str(iterations)+" iterations", 'sturm_histograms\histogram_tn_sH5f_2_sturms.pdf');
histogram_f(errors(1:tols), [0], solver_names, "Focal length error frequency in "+int2str(iterations)+" iterations", 'sturm_histograms\histogram_f_sH5f_2_sturms.pdf');

solver_names = {};
for t=1:size(tol,2)
    solver_names{t} = ['sH5f_3_{s', num2str(tol(t)), '}'];
end
histogram_rot(errors(tols+1:tols*2), [0], solver_names, "Rotation error frequency in "+int2str(iterations)+" iterations", 'sturm_histograms\histogram_rot_sH5f_3_sturms.pdf');
histogram_tn(errors(tols+1:tols*2), [0], solver_names, "Translation error frequency in "+int2str(iterations)+" iterations", 'sturm_histograms\histogram_tn_sH5f_3_sturms.pdf');
histogram_f(errors(tols+1:tols*2), [0], solver_names, "Focal length error frequency in "+int2str(iterations)+" iterations", 'sturm_histograms\histogram_f_sH5f_3_sturms.pdf');
...
for t=1:length(tol)
    ...
    [Hss, Nss, fss] = mex_sH5f_2_sturm([q_d(:,1:5)',p_d(:,1:5)',c_d', [eval(['1e-',num2str(tol(t))]);0;0;0;0] ]);
    ...
    try
        ...
        error_sh5f_2_sturm  = [error_sh5f_2_sturm, [norm(ts(:,ind)-t_gt,2)/norm(t_gt,2), abs(acosd((trace(R_gt'*Rs(:,3*ind-2:3*ind))-1)/2)), abs(fs(ind)-f_gt)/f_gt]'];
    catch
        error_sh5f_2_sturm = [error_sh5f_2_sturm, [inf;inf;inf]];
    end
...
for t=1:length(tol)
    ...
    [Hss, Nss, fss] = mex_sh5f_3_sturm([q_d(:,1:5)',p_d(:,1:5)',c_d', [eval(['1e-',num2str(tol(t))]);0;0;0;0] ]);
    ...
    try
        ind = find( abs(arrayfun(@(i) norm([Rs(:,3*i-2:3*i), ts(:,i)]-[R_gt,t_gt],2), 1:size(ts,2))) == min(abs(arrayfun(@(i) norm([Rs(:,3*i-2:3*i), ts(:,i)]-[R_gt,t_gt],2), 1:size(ts,2)))), 1);
        error_sh5f_3_sturm = [error_sh5f_3_sturm, [norm(ts(:,ind)-t_gt,2)/norm(t_gt,2), abs(acosd((trace(R_gt'*Rs(:,3*ind-2:3*ind))-1)/2)), abs(fs(ind)-f_gt)/f_gt]'];
    catch
        error_sh5f_3_sturm = [error_sh5f_3_sturm, [inf;inf;inf]];
    end
...
errors = [error_sh5f_2_sturm, error_sh5f_3_sturm];
% disp(errors);
end
