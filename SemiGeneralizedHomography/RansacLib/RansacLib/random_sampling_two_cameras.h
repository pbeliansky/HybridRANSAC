#ifndef RANSACLIB_RANSACLIB_RANDOM_SAMPLING_FROM_TWO_H_
#define RANSACLIB_RANSACLIB_RANDOM_SAMPLING_FROM_TWO_H_
...
namespace ransac_lib {
    ...
    class RandomSamplingFromTwo {
        public:
        RandomSamplingFromTwo(const unsigned int random_seed, const Solver& solver)
            : num_data_(solver.num_data()), sample_size_(solver.min_sample_size()) 
        {
            rng_.seed(random_seed);
            uniform_dstr_.param(std::uniform_int_distribution<int>::param_type(0, num_data_ - 1));
        ...
        {
            std::vector<int> cams(data_ranges_.size());
            std::vector<double> P(data_ranges_.size(), 1.0);

            int first_cam = ChooseCam(P);
            P[first_cam] = 0.0;
            cams[first_cam] = 3;

            for (size_t i = 0; i < 3; i++) selected_cams[i] = first_cam;      ////std::static_cast<uint8>(first_cam)
        ...
        void DrawHybridSample(int sample_size, int start, int end, std::vector<int>* random_sample)
        {
            ...
            std::uniform_int_distribution<int> u_dstr(start, end);
...
#endif  // RANSACLIB_RANSACLIB_RANDOM_SAMPLING_FROM_TWO_H_
