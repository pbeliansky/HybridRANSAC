...
namespace sh5f_3
{
	inline int solver_sturm(Eigen::Matrix<double, 3, 5> q, Eigen::Matrix<double, 3, 5> p, Eigen::Matrix<double, 3, 5> c,
		std::vector<double>* fss, std::vector<Eigen::Matrix3d>* Hs, std::vector<Eigen::Vector3d>* Nss, double tol = 1e-10)
	{
		...
		auto coef = get_coeffs(data);	//coef could be just array

		double roots[3] = { 0 };

		int n_roots = pose_lib::sturm::bisect_sturm<3>(coef.data(), roots, tol);
