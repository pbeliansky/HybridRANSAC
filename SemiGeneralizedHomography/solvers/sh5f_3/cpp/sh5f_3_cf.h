...
namespace sh5f_3
{
	inline int solver_closed_form(Eigen::Matrix<double, 3, 5> q, Eigen::Matrix<double, 3, 5> p, Eigen::Matrix<double, 3, 5> c,
		std::vector<double>* fss, std::vector<Eigen::Matrix3d>* Hs, std::vector<Eigen::Vector3d>* Nss)
	{
		...
		int n_roots = solver_common::solve_cubic_real(coef(2) / coef(3), coef(1) / coef(3), coef(0) / coef(3), roots);
