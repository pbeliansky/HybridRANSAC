...
#include "../common/charpoly.h"
#include "../common/sturm.h"
...
namespace sh45_2
{
    ...
    inline MatrixXd solver_problem_sh45_2(const VectorXd& data, int& n_roots, double tol)
    {
        ...
        double p[1 + 16];
        Matrix<double, 16, 16> AMp = AM;
        charpoly_danilevsky_piv(AMp, p);
        double roots[16];

        n_roots = pose_lib::sturm::bisect_sturm<16>(p, roots, tol);

        fast_eigenvector_solver(roots, n_roots, AM, sols);
        
        Matrix<double, 3, 16> solsReal;
        solsReal.setZero();

        solsReal.block<2, 16>(0, 0) = sols.real();
            ...
            std::vector<Eigen::Vector3d>* N_h, int test5thPt, double tol)
    {
        ...
        MatrixXd roots = solver_problem_sh45_2(data, n_roots, tol);
