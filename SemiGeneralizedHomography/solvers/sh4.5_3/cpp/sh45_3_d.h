...
namespace sh45_3
{
    void fast_eigenvector_solver(double* eigv, int neig, Eigen::Matrix<double, 12, 12>& AM, Matrix<std::complex<double>, 2, 12>& sols) {
        static const int ind[] = { 3,4,7,11 };
        // Truncated action matrix containing non-trivial rows
        Matrix<double, 4, 12> AMs;
        double zi[5];
        ...
        for (int i = 0; i < neig; i++) {
            ...
            AA.col(0) = AMs.col(3);
            AA.col(1) = AMs.col(2) + zi[0] * AMs.col(4);
            AA.col(2) = AMs.col(1) + zi[0] * AMs.col(5) + zi[1] * AMs.col(6) + zi[2] * AMs.col(7);
            AA.col(3) = AMs.col(0) + zi[0] * AMs.col(8) + zi[1] * AMs.col(9) + zi[2] * AMs.col(10) + zi[3] * AMs.col(11);
            AA(0, 0) = AA(0, 0) - zi[0];
            AA(1, 1) = AA(1, 1) - zi[1];
            AA(2, 2) = AA(2, 2) - zi[3];
            AA(3, 3) = AA(3, 3) - zi[4];
            ...
            Matrix<double, 3, 1>  s = AA.leftCols(3).colPivHouseholderQr().solve(-AA.col(3));
            sols(0, i) = s(2);
            sols(1, i) = zi[0];
    ...
    inline Eigen::MatrixXd solver_problem_sh45_3(const Eigen::VectorXd& data, int& nroots, double tol)
    {
        ...
        double p[1 + 12];
        Eigen::Matrix<double, 12, 12> AMp = AM;
        charpoly_danilevsky_piv(AMp, p);
        double roots[12];
        nroots = pose_lib::sturm::bisect_sturm<12>(p, roots, tol);

        fast_eigenvector_solver(roots, nroots, AM, solsComplex);
    ...
    inline int solver_danilevsky(Eigen::Matrix<double, 3, 5> q, Eigen::Matrix<double, 3, 5> p,
        Eigen::Matrix<double, 3, 5> c, std::vector<Eigen::Matrix3d>* homographies,
        std::vector<Eigen::Vector3d>* N_h, int test5thPt, double tol)
    {
        ...
        Eigen::MatrixXd roots = solver_problem_sh45_3(data, n_roots, tol);
